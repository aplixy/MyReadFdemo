...
public class DocxOfficeItem2 implements IBaseOfficeItemRender {
	...
	public DocxOfficeItem2(String filePath, TextView textView) {
	...
	private void writeDOCXPicture(byte[] pictureBytes, StringBuilder riverBuilder) {
	...
	public void render() {
				...
				switch (evtType) {
					...
					case XmlPullParser.START_TAG:
						...
						if (tag.equalsIgnoreCase("pic")) {  //检测到标签  pict  图片
							...
							cachePict(xlsxFile, pictureIndex, riverBuilder);
		...
		Logger.w("render", "riverBuilder--->" + riverBuilder);

		Spanned spanned = Html.fromHtml(riverBuilder.toString(), new Html.ImageGetter() {
			@Override
